...
def get_user(user_id):
    user = first_or_404(db.User.query.filter(db.User.id == user_id), 'User')
...
def update_user(user_id):
    ...
    user.save()
    redirect(url_for(get_user, user_id=user.id))
...
def create_goal():
    ...
    for prop, value in props.iteritems():
        if prop == 'user':
            ...
            user = first_or_404(db.User.query.filter(db.User.id == user_id))
        elif prop == 'start_ts':
            ...
            abort(400, "Unsupported parameter '%s'")
    ...
    goal.save()
    redirect(url_for(get_goal, goal_id=goal.id))


@app.route('/api/goals/<int:goal_id>', methods=['GET'])
def get_goal(goal_id):
    goal = first_or_404(db.Goal.query.filter(db.Goal.id == goal_id), 'Goal')
...
def create_measurement():
    ...
    measurement.save()
    redirect(url_for(get_measurement, measurement_id=measurement.id))


@app.route('/api/measurements/<int:measurement_id>', methods=['GET'])
def get_measurement(measurement_id):
    measurement = first_or_404(db.Measurement.query.filter(db.Measurement.id == measurement_id))
...
def create_meal():
    ...
    meal.save()
    redirect(url_for(get_meal, meal_id=meal.id))


@app.route('/api/meals/<int:meal>', methods=['GET'])
def get_meal(meal_id):
    meal = first_or_404(db.Measurement.query.filter(db.Meal.id == meal_id))
...
def update_meal(meal_id):
    props = request.get_json(force=True)
    meal = first_or_404(db.Meal.query.filter(db.Meal.id == meal_id))
    ...
    meal.save()
    redirect(url_for(get_measurement, meal_id=meal.id))
...
def first_or_404(query, type=None):
    try:
        return query.first()
